...
import { findItemValidation, upsertCartItemValidation } from "../validation/cartItems.validation";
import { HTTPException } from "hono/http-exception";
import { calculateTotalPrice, parseError } from "../lib/utils";
...
cartItemsRoutes.put(
  "/",
  zValidator("json", upsertCartItemValidation, async (result, c) => {
...
cartItemsRoutes.get(
  "/:productSlug",
  zValidator("param", findItemValidation, async (result, c) => {
    if (!result.success) throw new HTTPException(400, { message: "Validation error.", cause: parseError(result.error) });
