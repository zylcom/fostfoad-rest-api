...
reviewsRoutes.post("/", zValidator("json", createReviewValidation, async (result, c) => {
    if (!result.success)
    ...
    if (countReview !== 0)
        throw new HTTPException(409, { message: "Review already exist." });
...
reviewsRoutes.put("/", zValidator("json", updateReviewValidation, async (result, c) => {
    if (!result.success)
        throw new HTTPException(400, { message: "Validation error.", cause: parseError(result.error) });
    ...
    const review = await prisma.review
        ...
        if (e instanceof Prisma.PrismaClientKnownRequestError) {
            throw new HTTPException(404, { message: e.meta?.cause });
        }
