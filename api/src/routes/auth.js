...
authRoutes.post("/login", zValidator("json", UserLoginSchema, async (result, c) => {
    if (!result.success) {
...
authRoutes.post("/register", zValidator("json", UserRegisterSchema, async (result, c) => {
    if (!result.success) {
        throw new HTTPException(400, { message: "Validation error.", cause: parseError(result.error) });
    }
    const { username, password, profile: { name }, phonenumber: { number }, } = result.data;
    try {
